...
def run_random_baseline(env, task_name: str, verbose: bool = True) -> Dict[str, Any]:
    ...
    HEURISTIC_ACTIONS = {
        ...
        "code_review": [
            Action(action_type="identify_issue", target="code", value="SQL injection vulnerability - user input is directly interpolated into the SQL query string using f-string formatting",
                   reasoning="The query uses f-string interpolation which allows malicious SQL injection attacks."),
            Action(action_type="identify_issue", target="code", value="Password is compared in plain text without hashing, which is a critical security vulnerability",
                   reasoning="Passwords should never be stored or compared in plain text; bcrypt or argon2 should be used."),
            Action(action_type="suggest_fix", target="code", value="Use parameterized queries with cursor.execute('SELECT * FROM users WHERE username=%s', (username,)) to prevent SQL injection",
                   reasoning="Parameterized queries are the standard defense against SQL injection attacks."),
            Action(action_type="suggest_fix", target="code", value="Hash passwords using bcrypt. Compare with bcrypt.checkpw(password, stored_hash) instead of plain text comparison",
                   reasoning="Password hashing is essential for security. bcrypt provides salted hashing with configurable work factor."),
